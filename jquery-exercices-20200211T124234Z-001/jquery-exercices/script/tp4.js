...
(function ($) {
	...
	$(document).ready(function(){
		...
		*/
		var tables = $("table").css("display","none"); // display none cache les tableaux
		//console.log(tables);
		...
		*/
		...
		$('td').hover(
			function(){
				...
				console.log($text);
				$loupe.text($text);
// = même chose mais en 1 phrase : $loupe.css('display','block').text($(this).text());
			},
			function(){
		...
		*/
		$('table').each(function(){
			var letableau = $(this);
			letableau.find('td').hover( // hover = survoler
				function(){
					var $td = $(this);
				...
				function(){
					letableau.find('*').removeClass('on').removeClass('active');
				}
			);
		});
